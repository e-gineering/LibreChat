...
Pushing the tag fires the **"Docker Images Build on Tag"** workflow. Watch it to completion rather
than polling:
...
Expect **~10 min** now that the build is amd64-only. (Historically it built amd64+arm64 and took
~55–60 min, and the first run often *hung* on the arm64 emulation — the workaround was to cancel and
rerun, after which it succeeded. Dropping arm64 removed both the time and the hang. If a build ever
hangs again: `gh run cancel "$RUN_ID" --repo e-gineering/LibreChat` then
`gh run rerun "$RUN_ID" --repo e-gineering/LibreChat`.)
...
> Only the tag build matters for a release. On `dev` pushes you may still see other workflows; the
> noisy/failing ones (`dev-branch-images`, GitNexus) are disabled in this fork — see
> `references/customizations.md` if a new one starts failing.
